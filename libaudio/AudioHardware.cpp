...
void AudioHardware::loadRILD(void)
{
    ...
    if (mSecRilLibHandle) {
        ...
        setTwoMicControl = (int (*)(HRilClient, TwoMicDevice, TwoMicState))
                              dlsym(mSecRilLibHandle, "SetTwoMicControl");
        bool rilFunctionsLoaded = setCallVolume && setCallAudioPath && setCallClockSync && setTwoMicControl;
...
status_t AudioHardware::setParameters(const String8& keyValuePairs)
{
    ...
    const char NOISE_SUPPRESSION_VALUE_FALSE[] = "false";

    key = String8(AUDIO_PARAMETER_KEY_BT_NREC);
    if (param.get(key, value) == NO_ERROR) {
        if (value == AUDIO_PARAMETER_VALUE_ON) {
            mBluetoothNrec = true;
        } else if (value == AUDIO_PARAMETER_VALUE_OFF) {
        ...
        param.remove(String8(AUDIO_PARAMETER_KEY_BT_NREC));
    }

    key = String8(AUDIO_PARAMETER_KEY_TTY_MODE);
    if (param.get(key, value) == NO_ERROR) {
        int ttyMode;
        if (value == AUDIO_PARAMETER_VALUE_TTY_OFF) {
            ttyMode = TTY_MODE_OFF;
        } else if (value == AUDIO_PARAMETER_VALUE_TTY_VCO) {
            ttyMode = TTY_MODE_VCO;
        } else if (value == AUDIO_PARAMETER_VALUE_TTY_HCO) {
            ttyMode = TTY_MODE_HCO;
        } else if (value == AUDIO_PARAMETER_VALUE_TTY_FULL) {
        ...
        param.remove(String8(AUDIO_PARAMETER_KEY_TTY_MODE));
    }
...
#if 0
    /* AUDIO_PARAMETER_KEY_HANDLE_FM is needed here! */
